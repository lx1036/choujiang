...
+function()
{
	...
	var rgbBubbleD = 222;	//三原色泡泡直径
	var imgD = 180;			//图像直径
	var margin = (rgbBubbleD - imgD)/2;    //图像div距离外层div边距
	var imgRadius = imgD / 2;
	...
	CreateBubble = function(img)
	{
		// var img = 'heart.png';
		var bub = new Bubble(img);
...
//                        explode(bub.styBox);
	...
	function Bubble(img)
	{
		...
		var oBox = document.body.appendChild(document.createElement("div"));
                oBox.setAttribute("onclick","largen(this)");
		styBox = oBox.style;
		styBox.position = "absolute";
		styBox.width = styBox.height = D + "px";
		...
		for(var i=0; i<4; i++)
		{
			...
			{
				if(_IE6_)
					sty.filter = "progid:DXImageTransform.Microsoft.AlphaImageLoader(src="+image+")";
				else{
					sty.width = sty.height = imgD + "px";
					sty.marginLeft = sty.marginTop = margin + "px";
					sty.backgroundImage = "url("+img+")";
//                                        sty.backgroundSize = imgD * 0.8 +"px";
                                        sty.backgroundSize = "cover";
					sty.backgroundRepeat = "no-repeat";
					sty.backgroundPosition = "center";
					sty.borderRadius = imgRadius + "px";
				}
				break;
			}
			...
			{
				//rgb三原色气泡
				sty.width = sty.height = rgbBubbleD + "px";
				sty.backgroundImage = "url(image/ch" + i + ".png)";
				arrFlt[i] = sty;
			}
			else
			{
				sty.filter = "alpha progid:DXImageTransform.Microsoft.AlphaImageLoader(src=image/ch" + i + ".png)";
